...
///////////////////////////////////code của tao///////////////////////////////////
    ...
    public function createPost($data){
        $sql = "INSERT INTO `post`(`user_id`, `category_id`, `title`, `content`, `status`, `permission`) VALUES ('" . $data['user_id'] . "','" . $data['category_id'] . "','" . $data['title'] . "','" . $data['content'] . "','1','".$data['permission']."')";
    ...
    public function getPostsByCategoryIdForGuest($category_id){
        $query = "SELECT * FROM `post` WHERE `category_id` = $category_id AND `permission` = 0";
        $result = $this->usequery($query);
        $data1 = [];
        if ($result) {
            while ($num = mysqli_fetch_assoc($result)) {
    ...
    public function getPostsByCategoryIdForUser($category_id, $user_id){
        $query = "SELECT * FROM `post` WHERE (`permission` = 1 OR `user_id` = $user_id) AND `category_id` = $category_id";
        $result = $this->usequery($query);
        $data1 = [];
        if ($result) {
            while ($num = mysqli_fetch_assoc($result)) {
    ...
    public function updatePost($data){
        $sql = "UPDATE `post` SET `category_id`='" . $data['category_id'] . "',`title`='" . $data['title'] . "',`content`='" . $data['content'] . "',`status`='" . $data['status']."',`permission`='".$data['permission']."' WHERE `id`= ".$data['id']." AND `user_id`='" . $data['user_id'] . "'";
        if($this->usequery($sql)){
            return true;
        }else{
            return false;
